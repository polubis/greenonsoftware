...
**4focus MVP** ma na celu dostarczenie takiego właśnie narzędzia, koncentrując się na **Macierzy Eisenhowera** jako podstawowej metodzie organizacji.
...
#### **US-TASK-004: Zmiana statusu zadania na "W trakcie" (PENDING)**
**Opis:** Jako użytkownik, chcę móc oznaczyć zadanie jako "**W trakcie**" (**PENDING**), kiedy rozpocznę nad nim pracę, aby śledzić aktywne zadania i umożliwić pomiar czasu ich wykonania.
**Kryteria akceptacji:**
...
*   **And:** Wizualna reprezentacja zadania na liście może się zmienić, aby odzwierciedlić status **PENDING**.
*   *(Uwaga: Dokładny mechanizm inicjacji statusu PENDING wymaga doprecyzowania - czy jest to krok manualny?)*
*(Odnosi się do: FR-TASK-004, FR-TASK-005, FR-TASK-006)*

#### **US-TASK-005: Zmiana statusu zadania na "Zakończone" (DONE)**
**Opis:** Jako użytkownik, chcę móc oznaczyć zadanie jako "**Zakończone**" (**DONE**), kiedy je ukończę, aby móc śledzić postępy i oznaczyć zadanie jako niewymagające dalszej uwagi.
**Kryteria akceptacji:**
*   **Given:** Użytkownik jest zalogowany i widzi zadanie ze statusem **PENDING** na liście.
...
*   **And:** Obliczony czas trwania zadania (od **PENDING** do **DONE**) zostanie wyświetlony przy zakończonym zadaniu.
*(Odnosi się do: FR-TASK-004, FR-TASK-005, FR-TASK-006)*
...
**Kryteria akceptacji:**
*   **Given:** Użytkownik jest zalogowany i widzi widok kalendarza (np. dzienny).
...
**Kryteria akceptacji:**
*   **Given:** Użytkownik jest zalogowany i widzi widok kalendarza.
...
**Kryteria akceptacji:**
*   **Given:** Użytkownik jest zalogowany i widzi listę zadań.
...
**Opis:** Jako użytkownik, chcę móc korzystać z aplikacji zarówno na moim telefonie komórkowym, jak i na komputerze stacjonarnym, a interfejs powinien dostosować się do rozmiaru ekranu, zapewniając wygodną obsługę.
**Kryteria akceptacji:**
*   **Given:** Użytkownik otwiera aplikację w przeglądarce na urządzeniu mobilnym (różne popularne rozdzielczości).
*   **Then:** Interfejs aplikacji jest czytelny i w pełni funkcjonalny, elementy są odpowiednio rozmieszczone i łatwe do kliknięcia/dotknięcia (**FR-UI-002**, **FR-UI-003**).
*   **Given:** Użytkownik otwiera aplikację w przeglądarce na komputerze stacjonarnym (różne popularne rozdzielczości).
*   **Then:** Interfejs aplikacji wykorzystuje dostępną przestrzeń, jest czytelny i w pełni funkcjonalny (**FR-UI-002**, **FR-UI-003**).
*   **And:** Wszystkie funkcjonalności (dodawanie, edycja, zmiana statusu, przeglądanie, filtrowanie, logowanie/wylogowanie) są dostępne i działają poprawnie na obu typach urządzeń (**FR-UI-001**).
...
Pomiar tych metryk pozwoli ocenić, czy **MVP** spełnia podstawowe potrzeby użytkowników i czy istnieje zainteresowanie dalszym rozwojem produktu.
